...
function Header() {
	...
	const navLinks = [
		...
		{ label: 'Vehicles', href: '/#vehicles-page' },
	...
	useEffect(() => {
		const sections = ['home-content', 'vehicles-page', 'about-page', 'contact-page']
			...
			<div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-8 lg:px-12">
				...
				<nav className="hidden items-center gap-7 md:flex" aria-label="Main navigation">
					{navLinks.map((link, index) => {
						const sectionId = ['home-content', 'vehicles-page', 'about-page', 'contact-page'][index]
						const isActive = activeSection === sectionId
						return <a key={link.href} href={link.href} className={`relative py-5 font-['Manrope'] text-xs font-semibold transition ${isActive ? activeNav : navText} ${isActive ? 'after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-[#1976ed]' : ''}`}>{link.label}</a>
					})}
				</nav>
				<div className="flex items-center gap-2 sm:gap-3">
					<button className={`grid h-9 w-9 place-items-center rounded-full transition ${isDark ? 'text-blue-100/75 hover:bg-white/10 hover:text-white' : 'text-slate-500 hover:bg-blue-50 hover:text-[#176bd4]'}`} aria-label="Search"><FaSearch className="text-sm" /></button>
			...
			<nav className={`overflow-hidden border-t px-5 transition-all duration-300 md:hidden ${menuOpen ? 'max-h-96 py-4 opacity-100' : 'max-h-0 py-0 opacity-0'} ${isDark ? 'border-white/10 bg-[#10274b]' : 'border-slate-200 bg-white'}`} aria-label="Mobile navigation">
				<div className="mx-auto flex max-w-7xl flex-col gap-1">
					{navLinks.map((link, index) => {
						const sectionId = ['home-content', 'vehicles-page', 'about-page', 'contact-page'][index]
						const isActive = activeSection === sectionId
						return <a key={link.href} className={`rounded-md px-3 py-3 font-['Manrope'] text-sm font-semibold transition ${isActive ? `${activeNav} bg-blue-500/10` : navText}`} href={link.href} onClick={() => setMenuOpen(false)}>{link.label}</a>
					})}
				</div>
			</nav>
		</header>
